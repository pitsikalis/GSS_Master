...
public class SmoothRandom
{
    public static Vector3 GetVector3(float speed)
    {
        ...
        return new Vector3(Get().Noise(time, 150.73F, 0.58F), Get().Noise(time, 653.94F, 0.58F), Get().Noise(time, 770.2F, 0.58F));
    }

    public static float Get(float speed)
    {
        float time = Time.time * 0.01F * speed;
        return Get().Noise(time * 0.01F, 15.7F, 0.65F);
    }

    private static Perlin Get()
    {
        if (f_Noise == null)
            f_Noise = new Perlin();
        return f_Noise;
    ...
    private static Perlin f_Noise;
    //private static Perlin p_Noise;
}
